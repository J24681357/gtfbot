...
module.exports = {
  ...
  usedinlobby: true,
  description: [
    "!replay - Displays the list of your saved replays.",
    "!replay [load] [(number)] - Loads a saved replay from the [(number)] associated with the list of saved replays.",
    "!replay [delete] [(number)] - Deletes a saved replay from the [(number)] associated with the list of saved replays.",
    "!replay [clear] - Clears all of your saved replays.",
  ],
  execute(msg, query, userdata) {
    ...
    function replay() {
      ...
      embed.setTitle("__Replay Theater: " + Object.keys(replaystats).length + " / " + require(gtf.GTF).replaylimit + " Replays__");
      ...
      if (query[0] == "delete") {
        success = true;
        var number = query[1];
        if (number <= 0 || isNaN(number) || number === undefined || number > replaystats.length) {
        ...
        msg.channel.send(embed).then(msg => {
          gtftools.interval(
            function () {
              ...
              var emojilist = [
                [emote.tracklogo, "trackgtfitness", race],
                [emote.cargrid, "gtfcargrid", grid],
              ];
