...
module.exports = {
  ...
  execute(msg, query, userdata) {
    ...
    embed.setTitle(emote.exp + "__GTF Level Milestones: " + Object.keys(require(gtf.LISTS).gtfexp).length + " Levels" + "__");
    ...
    if (query.length != 0) {
      ...
      if (query[0] == "info") {
        var number = query[1];
        if (number <= 0 || isNaN(number) || number === undefined || number > require(gtf.LISTS).gtfexp.length) {
