...
module.exports = {
  ...
  usedinlobby: false,
  description: ['!decal ["query"] - Return search results of Gran Turismo Sport decals from a ["query"].\nFetched from https://gtswiki.gt-beginners.net/decal/'],
  execute(msg, query, userdata) {
    ...
    var dict = {
      decal: { url: "https://gtswiki.gt-beginners.net/decal/?searchword=" + m + "&lang=us&type=0&noid=1" },
      car: { url: "https://gts.gt-beginners.net/livery/?cartag=&searchword=" + m + "&type=0&d=" },
      helmet: { url: "https://gts.gt-beginners.net/helmet/?searchword=" + m },
      suit: { url: "https://gts.gt-beginners.net/wear/?searchword=" + m },
    };
    //var psnid = "&noid=1"
    var psnid = "";
    ...
    msg.channel.send(embed).then(msg => {
      ...
      https.get(dict[type]["url"] + psnid, resp => {
        let data = "";
        resp.on("data", chunk => {
          data += chunk;
        });
        ...
        resp.on("end", () => {
          setTimeout(function () {
            ...
            var liveries = data
              .split("</tr>")
              .slice(1, 20)
              .filter(x => x.includes('<td class="title">'))
              .map(function (x) {
                var title = x.split('<td class="title">')[1].split("</td>")[0];
                var author = x.split('<td class="author">')[1].split("</td>")[0];
                var comment = x.split('<td class="comment">')[1].split("</td>")[0];
                if (type == "decal") {
                  var image = "https://gtswiki.gt-beginners.net/" + type + x.split('src=".')[1].split('" width=')[0];
                } else if (type == "car") {
                ...
                var link = x.split('<a href="')[1].split('" target=')[0];
                return [title, author, comment, image, link];
              });
            if (liveries.length == 0) {
              msg.delete({});
              require(gtf.EMBED).alert(
                { name: "❌ Error", description: "Query returned an error for any of these reasons:" + "\n\n" + "Query is invalid.\n" + "There were no decals found.\n" + "API excceeded the amount of results to process.\n" + "Website has got an error.", embed: "", seconds: 0 },
                msg,
                userdata
              );
              return;
            ...
            function update() {
              results = "[" + liveries[page][0] + "]" + "(" + liveries[page][4] + ")" + "\n" + "**Author:** " + liveries[page][1] + "\n" + "**Description:** " + liveries[page][2] + "\n" + "**Link:** " + liveries[page][4];
              embed.setDescription(results);
              if (thumbnail) {
                embed.image = [];
              ...
              embed.setFooter("Page: " + (page + 1) + " / " + liveries.length);
            }
            update();
